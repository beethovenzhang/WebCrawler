...
from datamodel.search.Yunfeiz1Puc1_datamodel import Yunfeiz1Puc1Link
...
@Deleter(Yunfeiz1Puc1Link)
@Getter(Yunfeiz1Puc1Link)
class DeleteFrontierFrame(IApplication):
    ...
    def update(self):
        ...
        ls = self.frame.get(Yunfeiz1Puc1Link)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(Yunfeiz1Puc1Link, l)
...
class Simulation(object):
    ...
    def __init__(self, address, port):
        ...
        objectless_connector = ObjectlessSpacetimeConnection(
            "ResetFrontier_Yunfeiz1Puc1".format(CrawlerFrame.app_id),
